...
def get_camera_parameters(env_name, obs_shape):
    ...
    if env_name == 'avd':
        K   = 84.0 / 21.0 # orig image size / feature map size
...
def convert_world2map(pose, map_shape, map_scale, angles):
    """
    Inputs:
        pose - (bs, 3) Tensor agent pose in world coordinates (x, y, theta)
    ...
    ref_on_map_x = torch.clamp((mw-1)/2 + y/map_scale, 0, mw-1).round().long()
    ref_on_map_y = torch.clamp((mh-1)/2 - x/map_scale, 0, mh-1).round().long()
